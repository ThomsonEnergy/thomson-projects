...
3. Once the site is created, go to **Site configuration > Environment variables** and add:
   ...
   - `CLAUDE_AGENT_KEY` — lets a scheduled/headless Claude Code check leave a comment on a bug/update report (via `add-agent-comment.js`) without a real logged-in user session, e.g. to ask a clarifying question before acting on something. Only reports someone has clicked **Approve** on (while Dev Mode is unlocked, on `/bugs.html`) ever reach `BUGS_AND_UPDATES.md` in the first place, so a scheduled check only ever sees and comments on things that were actually reviewed. Pick your own value here, same as `DEV_MODE_PASSWORD` - it's never shipped to the browser.
