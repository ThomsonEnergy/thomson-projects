...
- **Mobile-friendly entries table**: the timesheet table stacks into cards on narrow screens instead of overflowing sideways
- **Today's schedule quick clock-in**: if staff are rostered on the Schedule calendar for today, one-tap buttons appear above the manual job picker
- **Split a day's time across multiple cost centres**: for jobs with more than one stage, a finished time entry can be split across stages with linked sliders and a live proportional bar - move one, the others rebalance automatically, always adding to 100%. Splitting replaces the single entry with one entry per stage, each with its own share of the clocked time
- **Manual entry editing**: staff can edit or add a missed clock in/out entry themselves, not just live ones
